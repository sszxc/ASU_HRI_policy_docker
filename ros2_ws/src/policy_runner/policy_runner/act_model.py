...
weights from the `.ckpt`, normalization from `dataset_stats.pkl`) and its default
(non-`--temporal_agg`) action-chunking behavior: query the model once every
`chunk_size` calls, serve the buffered chunk open-loop in between.
"""
...
class ACTChunkPolicy:
    def __init__(self, ckpt_path, act_repo_root="~/act", device=None):
    ...
    def next_action(self, qpos, images_by_camera):
        ...
        if self._chunk is None or self._chunk_step >= self.chunk_size:
            ...
            self._chunk = all_actions.squeeze(0).cpu().numpy()
            self._chunk_step = 0
