...
- [x] ROS2 DDS discovery configured (`.bashrc` + `fastdds_profile.xml`,
      ...
      `left`/`top` camera mapping is resolved (confirmed with user
      2026-09-03: `camera100`=left, `camera102`=top, `camera101`=right/unused)
      -- see "Live data-availability monitor" below.
- [x] `act_infer_mujoco` ROS2 node: subscribes the live left/top cameras +
      `/joint_states`, runs ACT inference at a fixed 30Hz, and writes the raw
      action straight into a MuJoCo model's qpos for visualization -- no
      actuators/`mj_step`, no UDP output yet. Model-loading + inference path
      verified end-to-end against the real checkpoint on GPU inside the
      running container (see "ACT inference -> MuJoCo visualization" below);
      the live-ROS + MuJoCo-viewer path itself has **not** been run against
      the live rig yet (needs a container restart to pick up the new
      `Honda_proto5_description` mount -- see that section).
...
- **Config:** new `act_inference:` block in `config/topics.yaml` --
...
## Earlier design notes (superseded by the section above, kept for context)
